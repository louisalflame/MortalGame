...
**MortalGame 是基於 Unity 的卡牌遊戲專案**，採用 **MVC 架構模式**與**三層資料架構**設計，實現清晰的職責分離與高度可維護性。整個系統由五個核心層級組成，各司其職又密切協作。
...
### 資料流向
```
GameData (靜態配置) → Presenter (協調轉換) → GameView (視覺呈現)
                   ↓
               Scene (場景統籌) ← UI (工具支援)
```
